...
function Output = new_my_system_ID(Method,bInput,sHour,bWaterDraw,...
    NAnalysis);
...
for ii = 1:N_E
    ...
    % load the correspoinding file from the folder (which contains the
    % steady-state case and the external input one)
    %
    ...
    Output.y_diff{ii} = Results{ii}.Prec - ...
        SS_Prec{ii}.Prec(low_idx:high_idx);
